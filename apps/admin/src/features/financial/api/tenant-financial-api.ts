...
export async function fetchLinkPresets() {
  const { data } = await api.get<{ items: LinkPresetDto[] }>("/admin/tenants/me/link-presets");
  return data.items;
}
...
export async function createLinkPreset(body: {
  ...
  const { data } = await api.post<LinkPresetDto>("/admin/tenants/me/link-presets", body);
  return data;
}
...
export async function updateLinkPreset(
  ...
  const { data } = await api.put<LinkPresetDto>(`/admin/tenants/me/link-presets/${id}`, body);
  return data;
}

export async function deleteLinkPreset(id: string) {
  const { data } = await api.delete<{ ok: boolean }>(`/admin/tenants/me/link-presets/${id}`);
  return data;
}
