...
| `successUrl` | string (URL) | não | Redirecionamento após pagamento na fatura Asaas (`callback.successUrl` na cobrança). O **origin** tem de estar em **`tenant_public_web_origins`**; o domínio também nos dados comerciais Asaas. Ver [public-payment-links.md](./public-payment-links.md) (mesma regra). Se omitido, pode usar-se o URL predefinido no painel (`payment-success-redirect`). |
