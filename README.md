...
6. **Admin** — pasta `apps/admin`: copiar `.env.example` → `.env` (mantém `API_URL=.../api` **com** `/api`), `npm install`, `npm run dev` (porta 5173).
...
2. Sem `/api` no URL → corrige `API_URL` no `.env` do admin e reinicia o Vite.
